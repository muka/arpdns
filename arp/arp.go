...
// ScanARP send ARP scan request
func ScanARP(response chan [2]string) error {
	...
	searchIface := viper.GetString("iface")
	if searchIface == "" {
		return errors.New("iface config is missing")
	}

	ifaces := make([]net.Interface, 0)
	for _, iface := range allIfaces {
		if iface.Name == searchIface {
			ifaces = append(ifaces, iface)
		}
	}

	if len(ifaces) == 0 {
		return fmt.Errorf("Iface %s not found", searchIface)
	}
	...
	for _, iface := range ifaces {
		...
		go func(iface net.Interface) {
			defer wg.Done()
			if err := scan(&iface, response); err != nil {
...
func scan(iface *net.Interface, response chan [2]string) error {
	...
	go readARP(handle, iface, stop, response)
	defer close(stop)
	for {
...
func readARP(handle *pcap.Handle, iface *net.Interface, stop chan struct{}, response chan [2]string) {
	...
	for {
		...
		case packet = <-in:
			...
			// fmt.Printf("%s -> %s\n", ip, hwaddr)
			response <- [2]string{ip.String(), hwaddr.String()}
		}
	}
}
